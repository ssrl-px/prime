...
def main(pr_pickle_file):
    ...
    if pres is not None:
        ...
        obs_asu = obs_asu.customized_copy(
            data=obs_asu.data() / partiality, sigmas=obs_asu.sigmas() / partiality
        )
        # group by similar indices
        obs_uniq = obs_asu.merge_equivalents().array()
        matches_uniq = miller.match_multi_indices(
            miller_indices_unique=obs_uniq.indices(), miller_indices=obs_asu.indices()
        )
        ...
        for token in poly_tokens:
            ...
            #    print ind, I, sigI, dI
        if len(obs_I) > 0:
            print pres.pickle_filename, "%6.2f" % (
                flex.sum(delta_I) * 100 / flex.sum(flex.abs(obs_I))
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("data", metavar="DATA", help="Path to post-refined pickles")
    ...
    cc_results = pool_map(iterable=frames, func=main, processes=n_proc)

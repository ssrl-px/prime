...
def calculate_centroid(sase_pickle_file, start_ev, end_ev):
    ...
    if n_pixels:
        ...
        for key in sase_pickle.keys():
            sase_centroids[key] = np.sum(
                bin_range * np.array(sase_pickle[key])
            ) / np.sum(np.array(sase_pickle[key]))
...
def compare_wavelength(
    img_pickle_dir, sase_spectrum, sase_centroids, sase_lambda_centroids, bin_range
):
    # read sase pickle
    img_set = {}
    for img_fname in os.listdir(img_pickle_dir):
        if img_fname.endswith(".pickle"):
            img_pickle = pickle.load(
                open(os.path.join(img_pickle_dir, img_fname), "rb")
            )
            img_set[img_fname] = 12398.4187 / img_pickle["WAVELENGTH"]
            img_fname_only = img_fname.split(".")[0]
            if img_fname_only in sase_centroids:
                print img_fname, img_set[img_fname], img_pickle[
                    "WAVELENGTH"
                ], sase_centroids[img_fname_only], sase_lambda_centroids[
                    img_fname_only
                ], img_set[
                    img_fname
                ] - sase_centroids[
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compare image-header wavelength with the calculated centroid of energy spectrum"
    )
    parser.add_argument("img_pickle_dir", metavar="dir", help="Path to image pickles")
    parser.add_argument(
        "sase_pickle_file",
        metavar="sase",
        help="Pickle file storing sase spectrum (see prime.get_SASE)",
    )
    parser.add_argument(
        "start_ev",
        metavar="start (eV)",
        type=int,
        help="Lower limit of the energy range (eV)",
    )
    parser.add_argument(
        "end_ev",
        metavar="end (ev)",
        type=int,
        help="Upper limit of the energy range (eV)",
    )
    parser.add_argument(
        "mode", metavar="mode", help="c for comparison, w for write-out only "
    )

    args = parser.parse_args()
    sase_spectrum, sase_centroids, sase_lambda_centroids, bin_range = calculate_centroid(
        args.sase_pickle_file, args.start_ev, args.end_ev
    )
    if args.mode == "c":
        compare_wavelength(
            args.img_pickle_dir,
    ...
    pickle.dump(
        sase_lambda_centroids, open(sase_pickle_filename + "_lambda.pickle", "wb")
    )

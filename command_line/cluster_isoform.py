...
def solve_with_mtz_mproc(args):
    ...
    cluster_id, txt_out = isoch.calc_cc(
        frame_no, pickle_filename, main_obs, iparams, miller_array_ref_set
    )
    print(txt_out)
...
def calculate_r_mproc(args):
    ...
    isoch = isoform_cluster_handler()
    r_set, main_obs, txt_out = isoch.calc_r(
        frame_no, pickle_filename, obs_main, obs_list
    )
    print(txt_out)
...
def write_out_solutions(iparams, sol_pickle):
    cluster_files = [
        os.path.join(iparams.run_no, "cluster_" + str(i) + ".lst")
...
class isoform_cluster_run_handler(object):
    ...
    def get_observation_set(self, iparams, frame_files, n_sel_frames):
        ...
        frames = [
            ...
            for i in random.sample(range(n_frames), n_sel_frames)
        ]
    ...
    def run(self, args):
        ...
        print(txt_out_input)
        with open(os.path.join(iparams.run_no, self.module_name, "log.txt"), "w") as f:
        ...
        if iparams.isoform_cluster.isorefin:
            ...
            if miller_array_ref_set:
                frames = [
                    (i, frame_files_sel[i], obs_list[i], iparams, miller_array_ref_set)
                ...
                txt_out = (
                    "Cluster images with given "
                    + str(len(miller_array_ref_set))
                    + " mtz files completed. Use cluster_0.lst - cluster_k.lst (for k clusters) for merging.\n"
                )
                print(txt_out)
                with open(
                    os.path.join(iparams.run_no, self.module_name, "log.txt"), "a"
                ) as f:
                    f.write(txt_out)
            return
        ...
        frames = [
            (i, frame_files_sel[i], obs_list[i], obs_list)
        ...
        frame_files_sel = []
        obs_sel = []
        for frame_file, r_set, obs in zip(
            frame_files_sorted, r_matrix_sorted, obs_list_sorted
        ):
        ...
        frames = [
            (i, frame_files_sel[i], obs_sel[i], obs_sel)
            for i in range(len(frame_files_sel))
        ]
        ...
        sol_pickle, cluster_files = isoch.assign_cluster(
            frame_files_sel, labels, k, os.path.join(iparams.run_no, self.module_name)
        )
        ...
        if n_frames > iparams.isoform_cluster.n_selected_frames:
            ...
            frame_files_remain_sel, obs_remain_sel_list = self.get_observation_set(
                iparams, frame_files_remain, len(frame_files_remain)
            )
            frames = [
                (
                    i,
                    frame_files_remain_sel[i],
                    obs_remain_sel_list[i],
                    iparams,
                    miller_array_ref_set,
                )
        ...
        txt_out += txt
        print(txt)
        with open(os.path.join(iparams.run_no, self.module_name, "log.txt"), "a") as f:
            f.write(txt_out)

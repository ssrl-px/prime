...
def main(exp, run_from, run_to):
    ...
    for run in range(run_from, run_to):
        ...
        for evt in events:
            ...
            if feeSpec_evt is not None:
                ...
                if (
                    fname_value.find("test") == -1
                    and fname_value.strip() != ""
                    and fname_value.find("image") == -1
                ):
    ...
    pickle.dump(
        e_dict,
        open(exp + "_" + str(run_from) + "_" + str(run_to) + "_sase.pickle", "wb"),
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DAQ Control IOC info XTC dumper")
    parser.add_argument("exp", metavar="EXP", help="the experiment number")
    parser.add_argument(
        "run_from", metavar="RUN FROM", type=int, help="the starting run number"
    )
    parser.add_argument(
        "run_to", metavar="RUN TO", type=int, help="the ending run number"
    )

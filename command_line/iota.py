...
def generate_input(gs_params):
    ...
    mp_input_list, mp_output_list = inp.make_mp_input(input_list, gs_params, gs_range)
...
def advanced_input(gs_range, gs_params):
    ...
    mp_trial_list, mp_trial_output_list = inp.make_mp_input(
        trial_list, gs_params, gs_range
    )
    ...
    if gs_params.advanced.random_sample.flag_on == True:
        summary.append(
            "raw images processed:         {}".format(
                gs_params.advanced.random_sample.number
            )
        )
    else:
...
def single_image_mode(gs_params):
    ...
    tmp_output_dir = os.path.abspath("{0}/tmp_{1}" "".format(output_dir, img_filename))
    ...
    for sig_height in range(h_min, h_max + 1):
        for spot_area in range(a_min, a_max + 1):
            mp_item = [current_img, sig_height, sig_height, spot_area, output_dir]
...
if __name__ == "__main__":
    ...
    # generate input
    gs_range, input_list, input_dir_list, output_dir_list, log_dir, logfile, mp_input_list, mp_output_list = generate_input(
        gs_params
    )

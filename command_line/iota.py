...
def iota_start(txt_out, gs_params, gs_range, input_dir_list, input_list, mp_input_list):
    ...
    if gs_params.grid_search.flag_on:
        inp.main_log(
            logfile,
            "{:-^100} \n\n".format(
                " TARGET FILE ({}) CONTENTS " "".format(gs_params.target)
            ),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", action="store_true", help="Convert raw images to pickles and stop"
    )
    parser.add_argument(
        "-d",
        action="store_true",
    ...
    if args.path == None:
        ...
        if os.path.exists(carg):
            ...
            # If user provided a data folder
                ...
                gs_params, txt_out = inp.auto_mode(
                    os.path.abspath(os.path.curdir), os.path.abspath(carg), now
                )
    ...
    if gs_params.grid_search.flag_on and gs_params.advanced.convert_images:
        ...
        if img_check == "image" or img_check == "raw pickle":
            ...
            beam_center = [
                gs_params.advanced.beam_center.x,
                gs_params.advanced.beam_center.y,
            ]
    ...
    gs_range, input_dir_list, output_dir_list, log_dir, logfile, mp_input_list, mp_output_list = inp.generate_input(
        gs_params, input_list, input_folder
    )

    # Print input image list and blank image list to files
    with open(
        os.path.abspath("{}/input_images.lst".format(gs_params.output)), "w"
    ) as lf:
        for input_file in input_list:
            lf.write("{}\n".format(input_file))

    with open(
        os.path.abspath("{}/blank_images.lst".format(gs_params.output)), "w"
    ) as bf:
    ...
    sg, uc, out_file = ia.unit_cell_analysis(
        gs_params.advanced.cluster_threshold,
        logfile,

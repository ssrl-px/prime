...
def index_mproc_wrapper(current_img):
    return gs.integration("grid", current_img, len(mp_input_list), log_dir, gs_params)


# Multiprocessor wrapper for selection module after grid search
def sel_grid_mproc_wrapper(output_entry):
    return ps.best_file_selection(
        "grid", gs_params, output_entry, log_dir, len(mp_output_list)
    )


# Multiprocessor wrapper for final integration module
def final_mproc_wrapper(current_img):
    return gs.integration("final", current_img, len(sel_clean), log_dir, gs_params)
...
if __name__ == "__main__":
    ...
    print "       II           O    O            TT           A       A          "
    print "       II           O    O            TT          A         A         "
    print "     IIIIII          OOOO             TT         A           A   v{}" "".format(
        iota_version
    )

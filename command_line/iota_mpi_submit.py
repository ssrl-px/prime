...
def copy_target(target, dest_dir, root_name):
    """Copy a phil file to a directory, and all of its included phil files.
    Recurively copies the files and changes the includes statements to match
    the new file names.
    ...
    for line in open(target).readlines():
        if "include" in line:
            inc_str, include_type, sub_target = (
                line.strip().split()
            )  # Example: include file cxi-8.2.phil
            if include_type != "file":
                raise Sorry(
                    "Include isn't a file"
                )  # FIXME look up what other values are possible here
            ...
            copy_target(
                os.path.join(os.path.dirname(target), sub_target),
                dest_dir,
                sub_target_root_name,
            )
            num_sub_targets += 1
        f.write(line)
...
class Script(object):
    ...
    def run(self):
        ...
        if params.input.run_num is None:
            ...
            print "Submitting run %d as iota job using param file %s" % (
        ...
        if params.input.run_num is None:
            ...
            rundir = os.path.join(
                params.output.output_dir, "r%04d" % params.input.run_num
            )
        ...
        os.mkdir(stdoutdir)
        if (
            params.output.split_logs
        ):  # test parameter for split_log then open and close log file and loop over nprocs
        ...
        for line in open(params.input.param).readlines():
            ...
            if "trial_id" in line:
                ...
                if not os.path.exists(val):
                    raise Sorry(
                        "One of the xtal_target files in the cfg file doesn't exist: %s"
                        % val
                    )
        ...
        if params.mp.method == "mpi":
            command_process = "bsub -a mympi -n %d -o %s -q %s %s %s --mpi process" % (
                params.mp.nproc,
                os.path.join(stdoutdir, "log.out"),
                params.mp.queue,
                params.input.dispatcher,
                param_path,
            )
            ...
            command_process = "qsub -o %s -d %s %s" % (
                os.path.join(stdoutdir, "log.out"),
                trialdir,
                submit_path,
            )
            ...
            psdmenv_path = os.path.join(
                os.environ.get("SIT_ROOT", ""), "etc", "ana_env.sh"
            )
            ...
            command_process = "qsub -o %s %s %s" % (
                os.path.join(stdoutdir, "log.out"),
                params.mp.custom.extra_args,
                submit_path,
            )

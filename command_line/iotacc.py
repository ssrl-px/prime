...
def get_Eoc_corrected_observations(pickle_filename, iparams):
    """Read pickle file name and output Eoc corrected original miller array
    object."""
    ...
    # Filter weak
    i_sel = (
        observations_original.data() / observations_original.sigmas()
    ) > iparams.iotacc.sigma_min
    observations_original = observations_original.customized_copy(
...
def calc_cc(miller_array_ref, miller_array_obs):
    ...
    I_ref = flex.double(
        [miller_array_ref.data()[pair[0]] for pair in matches_ref.pairs()]
    )
    I_o = flex.double(
        [miller_array_obs.data()[pair[1]] for pair in matches_ref.pairs()]
    )
...
def select_best_by_cc_mproc(shot_no, cryst_id, iparams):
    ...
    cryst_id_shot_no = (
        cryst_id + "_0_" + str(shot_no).zfill(iparams.iotacc.LEN_SHOT_SEQ)
    )
                    ...
                    if good_unit_cell(
                        ...
                        try:
                            ...
                            cc_eoc, n_refl_eoc = calc_cc(
                                miller_array_ref, observations_Eoc_corrected_asu
                            )
                            ...
                            cc_iso_eoc, n_refl_iso_eoc = calc_cc(
                                miller_array_iso, observations_Eoc_corrected_asu
                            )
    ...
    if len(pickle_filename_list) > 0:
        ...
        cc_thres = flex.max(cc_list) - (
            flex.max(cc_list) * iparams.iotacc.percent_top_cc / 100
        )
        ...
        pickle_filename_out = (
            iparams.run_no
            + "/"
            + iparams.iotacc.set_id
            + "_"
            + pickle_filename_sel_only
        )

        # shutil.copyfile(pickle_filename_sel, pickle_filename_out)
        txt_out += (
            "Select --> CC_thres=%6.2f Best CC=%6.2f N_refl=%4.0f "
            % (cc_thres, cc_sel, n_refl_sel)
            + pickle_filename_out
            + "\n"
        )
        flag_success = True

...
def master(frame_token, iparams, activity):
    if activity == "scale":
        ...
        for i in indices:
            i_end = (
                i + n_batch if i + n_batch < len(frame_objects) else len(frame_objects)
            )
        ...
        for i in indices:
            ...
            rankreq = comm.recv(source=MPI.ANY_SOURCE)
            comm.send(
                (activity, (frame_objects[i:i_end], iparams, avg_mode)), dest=rankreq
            )
    elif activity == "merge":
        frame_objects, avg_mode = frame_token
        ...
        for i in indices:
            ...
            sel = flex.bool(
                [sel_l and sel_h for sel_l, sel_h in zip(cpo[1] >= i, cpo[1] < i_end)]
            )
            ...
            comm.send(
                (
                    ...
                    (
                        ...
                        iparams,
                        miller_array_ref,
                        pres_dict[frame_files[i]]
                        if frame_files[i] in pres_dict
                        else None,
                        avg_mode,
                    ),
                ),
    ...
    while True:
        ...
        if activity == "scale":
            ...
            for frame_index, frame_file in enumerate(frame_files):
                pres, _ = prh.scale_frame_by_mean_I(
                    frame_index, frame_file, iparams, 0, "average"
                )
                result.append(pres)
        elif activity == "pre_merge":
            ...
            pres, _ = prh.postrefine_by_frame(
                frame_no, frame_file, iparams, miller_array_ref, pres_in, avg_mode
            )
            result.append(pres)
    return result
    ...
    if rank == 0:
        ...
        selections = flex.bool(
            [
                False
                if (
                    math.isnan(r0) or math.isinf(r0) or math.isnan(r1) or math.isinf(r1)
                )
                else True
...
def run(argv):
    ...
    for i_iter in range(n_postref_cycle):
        ...
        comm.Barrier()
        results, mdh = merge(
            result, iparams, "", "postref_cycle_%d" % (i_iter + 1), avg_mode
        )

...
def scale_frame_by_mean_I_mproc(
    frame_no, frame_files, iparams, mean_of_mean_I, avg_mode
):
    from prime.postrefine import postref_handler

    prh = postref_handler()
    pres = prh.scale_frame_by_mean_I(
        frame_no, frame_files[frame_no], iparams, mean_of_mean_I, avg_mode
    )
    return pres


def postrefine_by_frame_mproc(
    frame_no, frame_files, iparams, miller_array_ref, pres_results, avg_mode
):
    from prime.postrefine import postref_handler
    ...
    pres = prh.postrefine_by_frame(
        frame_no, frame_files[frame_no], iparams, miller_array_ref, pres_in, avg_mode
    )
    return pres
...
if __name__ == "__main__":
    ...
    def scale_frame_by_mean_I_mproc_wrapper(arg):
        return scale_frame_by_mean_I_mproc(
            arg, frame_files, iparams, mean_of_mean_I, avg_mode
        )
    ...
    if len(observations_merge_mean_set) > 0:
        ...
        if prep_output is not None:
            ...
            if True:
                ...
                miller_index, I_avg, sigI_avg, stat, I_avg_two_halves_tuple, txt_obs_out, txt_reject_out = (
                    calc_average_I_result
                )
                I_avg_even, I_avg_odd, I_avg_even_h, I_avg_odd_h, I_avg_even_k, I_avg_odd_k, I_avg_even_l, I_avg_odd_l = (
                    I_avg_two_halves_tuple
                )
            ...
            iparams.indexing_ambiguity.index_basis_in = (
                iparams.run_no + "/mean_scaled_merge.mtz"
            )
    ...
    if iparams.fraction_r_free > 0:
        ma_ref_r_free_flags = miller_array_ref.generate_r_free_flags(
            fraction=iparams.fraction_r_free, n_shells=iparams.n_bins
        )
        ma_indices_for_test = miller_array_ref.indices().select(
            ma_ref_r_free_flags.data()
        )
        txt_r_free_out = ""
        for i in range(len(ma_indices_for_test)):
            txt_r_free_out += (
                str(ma_indices_for_test[i][0])
                + " "
                + str(ma_indices_for_test[i][1])
                + " "
                + str(ma_indices_for_test[i][2])
                + "\n"
            )
    ...
    for i_iter in range(n_iters):
        ...
        if len(postrefine_by_frame_good) > 0:
            ...
            if prep_output is not None:
                ...
                # reset index_basis_in
                iparams.indexing_ambiguity.index_basis_in = (
                    iparams.run_no + "/postref_cycle_" + str(i_iter + 1) + "_merge.mtz"
                )

                # calculate R and Rfree
                R_work, R_free = (0, 0)
                if ma_ref_r_free_flags is not None:
                    ma_ref_r_work_indices = ma_ref_r_free_flags.indices().select(
                        ma_ref_r_free_flags.data() == False
                    )
                    ma_ref_r_free_indices = ma_ref_r_free_flags.indices().select(
                        ma_ref_r_free_flags.data()
                    )
                    ...
                    I_pr_match = flex.double(
                        [miller_array_pr.data()[pair[1]] for pair in matches.pairs()]
                    )
                    miller_indices_match = flex.miller_index(
                        [miller_array_pr.indices()[pair[1]] for pair in matches.pairs()]
                    )
                    ...
                    I_ref_work = flex.double(
                        [I_ref_match[pair[0]] for pair in matches.pairs()]
                    )
                    I_pr_work = flex.double(
                        [I_pr_match[pair[0]] for pair in matches.pairs()]
                    )
                    ...
                    I_ref_free = flex.double(
                        [I_ref_match[pair[0]] for pair in matches.pairs()]
                    )
                    I_pr_free = flex.double(
                        [I_pr_match[pair[0]] for pair in matches.pairs()]
                    )

                    R_work = math.sqrt(
                        np.sum((I_ref_work - I_pr_work) ** 2) / np.sum(I_ref_work ** 2)
                    )
                    R_free = math.sqrt(
                        np.sum((I_ref_free - I_pr_free) ** 2) / np.sum(I_ref_free ** 2)
                    )
                    txt_R_out = " Rwork:                   %12.2f%% REFL: %6.2f%%\n" % (
                        R_work * 100,
                        (len(I_ref_work) / len(miller_indices_match)) * 100,
                    )
                    txt_R_out += (
                        " Rfree:                   %12.2f%% REFL: %6.2f%%\n"
                        % (
                            R_free * 100,
                            (len(I_ref_free) / len(miller_indices_match)) * 100,
                        )
                    )

...
if __name__ == "__main__":
    ...
    if len(observations_merge_mean_set) > 0:
        ...
        if prep_output is not None:
            ...
            if (
                iparams.averaging_engine == "python"
                or iparams.averaging_engine == "both"
            ):
            ...
            if iparams.averaging_engine == "cpp" or iparams.averaging_engine == "both":
                ...
                I_odd2 = flex.double()
                if (
                    iparams.averaging_engine == "cpp"
                ):  # don't save the log twice if using 'both'
                    txt_out_verbose += (
            ...
            if iparams.averaging_engine == "both":
                ...
                assert [
                    approx_equal(a, b, 1e-20, out=None)
                    for a, b in zip(miller_indices_merge, miller_indices_merge2)
                ].count(True) == len(miller_indices_merge)
                assert [
                    approx_equal(a, b, 1e-20, out=None)
                    for a, b in zip(I_merge, I_merge2)
                ].count(True) == len(I_merge)
                assert [
                    approx_equal(a, b, 1e-20, out=None)
                    for a, b in zip(sigI_merge, sigI_merge2)
                ].count(True) == len(sigI_merge)
                assert [
                    approx_equal(a, b, 1e-20, out=None) for a, b in zip(I_even, I_even2)
                ].count(True) == len(I_even)
                assert [
                    approx_equal(a, b, 1e-20, out=None) for a, b in zip(I_odd, I_odd2)
                ].count(True) == len(I_odd)
                assert [
                    approx_equal(a, b, 1e-20, out=None)
                    for a, b in zip(stat_all[0], stat_all2[0])
                ].count(True) == len(stat_all[0])
                assert [
                    approx_equal(a, b, 1e-20, out=None)
                    for a, b in zip(stat_all[1], stat_all2[1])
                ].count(True) == len(stat_all[1])
                assert [
                    approx_equal(a, b, 1e-20, out=None)
                    for a, b in zip(stat_all[2], stat_all2[2])
                ].count(True) == len(stat_all[2])
                assert [
                    approx_equal(a, b, 1e-20, out=None)
                    for a, b in zip(stat_all[3], stat_all2[3])
                ].count(True) == len(stat_all[3])
                assert [
                    approx_equal(a, b, 1e-20, out=None)
                    for a, b in zip(stat_all[4], stat_all2[4])
                ].count(True) == len(stat_all[4])
        ...
        if len(postrefine_by_frame_good) > 0:
            ...
            if prep_output is not None:
                ...
                if iparams.averaging_engine == "both":
                    ...
                    assert [
                        approx_equal(a, b, 1e-20, out=None)
                        for a, b in zip(I_even, I_even2)
                    ].count(True) == len(I_even)
                    assert [
                        approx_equal(a, b, 1e-20, out=None)
                        for a, b in zip(I_odd, I_odd2)
                    ].count(True) == len(I_odd)
                    assert [
                        approx_equal(a, b, 1e-20, out=None)
                        for a, b in zip(stat_all[0], stat_all2[0])

...
def merge_frames(pres_set, iparams, avg_mode="average", mtz_out_prefix="mean_scaled"):
    ...
    if pres_set:
        ...
        if prep_output is not None:
            ...
            stat_all = [
                [_s0, _s1, _s2, _s3, _s4]
                for _s0, _s1, _s2, _s3, _s4 in zip(
                    s0.select(selections),
                    s1.select(selections),
                    s2.select(selections),
                    s3.select(selections),
                    s4.select(selections),
                )
            ]
            ...
            miller_array_ref, txt_merge_mean_table = write_output(
                miller_index.select(selections),
                I_avg.select(selections),
                sigI_avg.select(selections),
                stat_all,
                (
                    I_avg_even.select(selections),
                    I_avg_odd.select(selections),
                    I_avg_even_h.select(selections),
                    I_avg_odd_h.select(selections),
                    I_avg_even_k.select(selections),
                    I_avg_odd_k.select(selections),
                    I_avg_even_l.select(selections),
                    I_avg_odd_l.select(selections),
                ),
                iparams,
                uc_mean,
                wavelength_mean,
                iparams.run_no + "/" + mtz_out_prefix,
                avg_mode,
            )

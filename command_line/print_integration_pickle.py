...
if __name__ == "__main__":
    ...
    data, hklrefin, pixel_size_mm, target_unit_cell, target_space_group, target_anomalous_flag, flag_plot, d_min, d_max, n_residues = read_input(
        args=sys.argv[1:]
    )
    ...
    for pickle_filename in frame_files:
        ...
        swap_dict = {"test0.pickle": 0, "test1.pickle": 0}
        ...
        if target_unit_cell is not None:
            ...
            for (
                ...
                if sys_absent_flag[1]:
                    txt_out = "{9:40} {10:6.2f} {0:3} {1:3} {2:3} {3:3} {4:3} {5:3} {6:8.2f} {7:8.2f} {8:6.2f}".format(
                        miller_index_ori[0],
                        miller_index_ori[1],
                        miller_index_ori[2],
                        miller_index_asu[0],
    ...
    for (
        ...
        if diff_xy.length() < beam_thres:
            flag_good_beam = True
        if (cc_bin_low > cc_bin_low_thres) or hklrefin is None:
            flag_good_cc = True
        if flag_good_beam:
        ...
        if flag_good_beam and flag_good_cc and flag_good_unit_cell:
            ...
            cn_bad += 1
    print
    print "Images with delta_beam_xy > %6.2f mm (image name, xbeam, ybeam, cciso, delta_xy)" % (
        beam_thres
    )

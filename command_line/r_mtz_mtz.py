    ...
    if hklin is not None:
        ...
        for miller_array in miller_arrays:
            if miller_array.is_xray_amplitude_array():
                ...
                miller_array_converted_to_amplitude = (
                    miller_array.enforce_positive_amplitudes()
                )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        print(
            "Only linear scale will be performed. Set flag_b=True to include B-factor scaling."
        )
        print(
            "Usage: prime.r_mtz_mtz hkla=reflection1.mtz hklb=reflection2.mtz d_min=min_resolution d_max=max_resolution n_bins=no_of_bins flag_b=True_or_False."
        )
        exit()
    ...
    if flag_hkla_found and flag_hklb_found:
        ...
        # scale b to a
        miller_array_b_scaled = miller_array_a.scale(
            miller_array_b, resolution_dependent=flag_b
        )
        # get common sets

...
if __name__ == "__main__":
    ...
    data, hklrefin, pixel_size_mm, target_unit_cell, d_min, d_max = read_input(
        args=sys.argv[1:]
    )
    ...
    for pickle_filename in frame_files:
        ...
        if flag_good_unit_cell and len(spot_pred_x_mm) > 0:
            ...
            r0 = ph.calc_spot_radius(
                ...
                observations_pickle["wavelength"],
            )
            ...
            for i in range(len(observations.data())):
                ...
                if (
                    assigned_lambda[i] == 0
                    or assigned_lambda[i] == len(lambda_template) - 1
                ):
                    """print p_array[i,:], assigned_lambda[i]
                    plt.plot(lambda_set, p_array[i,:]) plt.title('Best
                    wavelength=%8.6f (i=%2.0f)
            ...
            n, bins, patches = plt.hist(
                assigned_lambda, 30, normed=0, facecolor="b", alpha=0.5
            )
            plt.show()

...
class indexing_ambiguity_handler(object):
    ...
    def run(self, args):
        ...
        if self.should_terminate(iparams, frame_files[0]):
            print(
                "No indexing ambiguity problem. Set index_ambiguity.mode = Forced and assigned_basis = list of basis formats to solve pseudo-twinning problem."
            )
            return None, iparams
        ...
        if iparams.indexing_ambiguity.index_basis_in is not None:
            if iparams.indexing_ambiguity.index_basis_in.endswith(".mtz"):
                ...
                if flag_ref_found == False:
                    print(
                        "Reference mtz file not found. Set indexing_ambiguity.index_basis_in = None to enable auto generate the solutions."
                    )
                    return None, iparams
                else:
                    frames = [
        ...
        with open(iparams.run_no + "/log.txt", "a") as f:
            f.write(txt_out)
        print(
            "Indexing Ambiguity Solver Elapsed Time (s) %10.2s" % (time.time() - start)
        )
        return sol_pickle, iparams

...
def main(img_fname, int_fname):
    """read image pickle and integration pickle then show predictions on the
    top of the diffraction pattern."""
    ...
    for ind, ind_asu, I, x, y in zip(
        obs.indices(), obs_asu.indices(), obs.data(), spot_pred_x_mm, spot_pred_y_mm
    ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "img", metavar="IMG", help="Diffraction pattern as a pickle file"
    )
    parser.add_argument("int", metavar="INT", help="Integration result pickle")

        ...
        if len(total_tmp_pickles) == 0:
            ...
            if len(acceptable_pickles) == 0:
                ps_logger.info(
                    ...
                    "in {1}:\n".format(len(total_tmp_pickles), tmp_dir)
                )
                ...
                for pickle in acceptable_pickles:
                    ...
                    sref = len(
                        [val for val in I_over_sigI if val >= gs_params.min_sigma]
                    )
                ...
                # Total reflections
                sel_pickle = best_by_reflections(
                    gs_params, acceptable_pickles, output_dir + "/best_by_total"
                )
                with open(
                    "{}/best_by_total.lst".format(os.path.abspath(gs_params.output)),
                    "a",
                ) as best_file:
                    best_file.write("{}\n".format(sel_pickle))
                ...
                # Strong reflections
                sel_pickle = best_by_strong(
                    gs_params, acceptable_pickles, output_dir + "/best_by_strong"
                )
                with open(
                    "{}/best_by_strong.lst".format(os.path.abspath(gs_params.output)),
                    "a",
                ) as best_file:
                    best_file.write("{}\n".format(sel_pickle))
                ...
                # x,y offset
                sel_pickle = best_by_offset(
                    acceptable_pickles, output_dir + "/best_by_offset"
                )
                with open(
                    "{}/best_by_offset.lst".format(os.path.abspath(gs_params.output)),
                    "a",
                ) as best_file:
                    best_file.write("{}\n".format(sel_pickle))

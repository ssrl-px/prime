...
class PRIMEBaseBackendDialog(BaseBackendDialog):
    def __init__(
        self, parent, content_style="normal", label_style="bold", *args, **kwargs
    ):
        BaseBackendDialog.__init__(
            self,
            parent,
...
class PRIMEAdvancedOptions(PRIMEBaseBackendDialog):
    ...
    def __init__(self, parent, phil=None, *args, **kwargs):
        PRIMEBaseBackendDialog.__init__(
            self,
            parent,
            backend_name="PRIME",
            phil=phil,
        ...
        self.p_scale_res = ct.OptionCtrl(
            self.prm_options,
        ...
        self.p_cryst_res = ct.OptionCtrl(
            self.prm_options,
            checkbox=True,
        ...
        self.p_rrange_res = ct.OptionCtrl(
            self.prm_options,
            checkbox=True,
        ...
        self.btn_synch_res = wx.Button(
            self.prm_options, label="Set Res. Limits to Postref. Scale"
        )
    ...
    def read_param_phil(self):
        ...
        self.scale_res.low.SetValue("{:4.2f}".format(self.pparams.scale.d_max))
        self.scale_res.high.SetValue("{:4.2f}".format(self.pparams.scale.d_min))
        self.merge_res.low.SetValue("{:4.2f}".format(self.pparams.merge.d_max))
        self.merge_res.high.SetValue("{:4.2f}".format(self.pparams.merge.d_min))
        ...
        self.p_cryst_res.toggle_boxes(
            flag_on=self.pparams.postref.crystal_orientation.flag_on
        )
        ...
        self.p_rrange_res.toggle_boxes(
            flag_on=self.pparams.postref.reflecting_range.flag_on
        )
...
class RecoveryDialog(BaseDialog):
    def __init__(
        ...
        unknown_bmp = bitmaps.fetch_icon_bitmap("actions", "status_unknown", size=16)

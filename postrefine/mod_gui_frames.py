...
class PRIMEWindow(IOTABaseFrame):
    def __init__(self, parent, id, title, prefix="prime"):
        ...
        self.tb_btn_analysis = self.add_tool(
            label="Recover",
            bitmap=("mimetypes", "text-x-generic-2"),
            shortHelp="Recover PRIME run",
        )
        self.tb_btn_run = self.add_tool(
            label="Run", bitmap=("actions", "run"), shortHelp="Run PRIME"
        )
    ...
    def onRecovery(self, e):
        ...
        if not os.path.isdir(p_folder):
            open_dlg = wx.DirDialog(
                self, "Choose the integration run:", style=wx.DD_DEFAULT_STYLE
            )
...
class SummaryTab(wx.Panel):
    def __init__(self, parent, pparams, info):
        ...
        self.btn_stats = ct.GradButton(
            self, bmp=line_bmp, label=" Statistical charts", size=(250, -1)
        )
        txt_bmp = bitmaps.fetch_icon_bitmap("mimetypes", "txt", scale=(24, 24))
        self.btn_table1 = ct.GradButton(
            self, bmp=txt_bmp, label=" Output Table 1", size=(250, -1)
        )
...
class PRIMERunWindow(IOTABaseFrame):
    ...
    def run(self):
        ...
        self.prime_process = thr.PRIMEThread(
            self, self.prime_file, self.out_file, command=self.command
        )

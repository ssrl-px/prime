...
from iota.components.gui.base import IOTABasePanel, IOTABaseFrame, IOTABaseScrolledPanel
...
class PRIMEInputWindow(IOTABasePanel):
    ...
    def __init__(self, parent, phil=None):
        ...
        self.project_title = ct.InputCtrl(
            self, label="Project Title: ", label_size=(140, -1)
        )
        # Output file box
        self.out_box = ct.InputCtrl(
            self,
        ...
        self.opt_spc_nproc = ct.SpinCtrl(
            self,
    ...
    def onInputBrowse(self, e):
        """On clincking the Browse button: show the DirDialog and populate
        'Input' box w/ selection."""
        inp_dlg = wx.DirDialog(
            self, "Choose the input directory:", style=wx.DD_DEFAULT_STYLE
        )
...
class PRIMEWindow(IOTABaseFrame):
    ...
    def __init__(self, parent, id, title, prefix="prime"):
        ...
        self.tb_btn_quit = self.add_tool(
            label="Quit", bitmap=("actions", "exit"), shortHelp="Exit PRIME"
        )

        self.tb_btn_prefs = self.add_tool(
            label="Preferences",
            bitmap=("apps", "advancedsettings"),
            shortHelp="PRIME GUI Settings",
        )
        self.add_toolbar_separator()
        self.tb_btn_load = self.add_tool(
            label="Load Script",
            bitmap=("actions", "open"),
        ...
        self.tb_btn_reset = self.add_tool(
            label="Reset", bitmap=("actions", "reload"), shortHelp="Reset Settings"
        )
        self.toolbar.AddSeparator()
        self.tb_btn_analysis = self.add_tool(
            label="Recover", bitmap=("actions", "list"), shortHelp="Recover PRIME run"
        )
        ...
        self.bottom_sizer.Add(
            line, pos=(0, 0), span=(1, 3), flag=wx.EXPAND | wx.TOP | wx.BOTTOM, border=5
        )

        opt_scope = self.prime_index.get_scopes(include=self.opt_scopes)
        self.option_panel = pct.PHILDialogPanel(
            self, scope=opt_scope, phil_index=self.prime_index
        )
        self.bottom_sizer.Add(
            self.option_panel, pos=(1, 0), span=(2, 1), flag=wx.EXPAND
        )
        ...
        self.bottom_sizer.Add(
            self.btn_run, pos=(2, 2), flag=wx.SHAPED | wx.ALIGN_BOTTOM
        )
    ...
    def open_options_dialog(self, phil_index, include=None, name=None, exclude=None):
        if not name:
        ...
        phil_scope = phil_index.get_scopes(include=include, exclude=exclude)
    ...
    def onPreferences(self, e):
        self.open_options_dialog(phil_index=self.prime_index, include=self.pref_scopes)

    def onPostRefOptions(self, e):
        self.open_options_dialog(phil_index=self.prime_index, include=self.pr_scopes)

    def onAdvancedOptions(self, e):
        not_adv = (
            self.main_window_scopes
            + self.opt_scopes
            + self.pref_scopes
            + self.pr_scopes
        )
        self.open_options_dialog(phil_index=self.prime_index, exclude=not_adv)
    ...
    def onRecovery(self, e):
        ...
        paths = [
            p for p in paths if (os.path.isdir(p) and os.path.basename(p).isdigit())
        ]
        ...
        if path_dlg.ShowModal() == wx.ID_OK:
            ...
            if recovery == 0:
                self.prime_run_window = PRIMERunWindow(
                    self,
                    -1,
                    title="PRIME Output",
                    params=self.pparams,
        ...
        if self.init_settings():
            ...
            if not os.path.isdir(source_dir):
                try:
                    ...
                    raise Sorry(
                        "PRIME ERROR: Could not create folder {}: {}"
                        "".format(source_dir, e)
                    )
...
class RuntimeTab(IOTABasePanel):
    def __init__(self, parent, params=None, *args, **kwargs):
        ...
        gsub = gridspec.GridSpecFromSubplotSpec(3, 1, subplot_spec=gsp[3], hspace=0)
        self.bcc_axes = self.prime_figure.add_subplot(gsub[0])
        self.bcomp_axes = self.prime_figure.add_subplot(gsub[1], sharex=self.bcc_axes)
        self.bmult_axes = self.prime_figure.add_subplot(gsub[2], sharex=self.bcc_axes)
        self.draw_axes()
    ...
    def draw_axes(self):
        ...
        self.cc_axes.set_xticks(
            [tick for tick in self.cc_axes.get_xticks() if tick % 1 == 0]
        )
        ...
        self.comp_axes.set_xticks(
            [tick for tick in self.comp_axes.get_xticks() if tick % 1 == 0]
        )
    ...
    def draw_plots(self, info, total_cycles):
        ...
        mult = self.mult_axes.scatter(
            cycles, mean_mult, marker="o", s=25, edgecolors="black", color="#feb24c"
        )
        labels = ["Completeness", "Multiplicity"]
        self.comp_axes.legend(
            [comp, mult], labels, loc="upper right", fontsize=9, fancybox=True
        )
        ...
        self.bcc_axes.bar(bins, binned_cc, color="#2b8cbe", alpha=0.5, width=1, lw=0)
        ...
        self.bcomp_axes.step(
            bins, info["binned_completeness"][-1], color="red", where="mid"
        )
        self.bmult_axes.clear()
        self.bmult_axes.bar(
            bins, info["binned_n_obs"][-1], alpha=0.5, color="#feb24c", width=1, lw=0
        )
        self.bmult_axes.step(
            bins, info["binned_n_obs"][-1], color="orange", where="mid"
        )
        ...
        self.rej_table.text(
            0, 0.85, txt, fontdict=font, transform=self.rej_table.transAxes, va="top"
        )
...
class SummaryTab(IOTABaseScrolledPanel):
    def __init__(self, parent, pparams, info, *args, **kwargs):
        ...
        self.tb1_box_grid.Add(self.tb1_table, pos=(0, 0), span=(3, 1), flag=wx.EXPAND)
        ...
        self.btn_stats = ct.GradButton(self, bmp=line_bmp, label=" Statistical charts")
        txt_bmp = bitmaps.fetch_icon_bitmap("mimetypes", "txt", scale=(24, 24))
        self.btn_table1 = ct.GradButton(self, bmp=txt_bmp, label=" Output Table 1")
    ...
    def show_plot(self):
        ...
        self.plot_window.place_and_size(
            set_by="parent", set_size=True, position=(25, 25)
        )
...
class PRIMERunWindow(IOTABaseFrame):
    ...
    def __init__(
        ...
        # Toolbar
        self.initialize_toolbar()
        self.tb_btn_abort = self.add_tool(
            label="Abort", bitmap=("actions", "stop"), shortHelp="Abort"
        )
        self.realize_toolbar()
        ...
        self.gauge_prime = wx.Gauge(self.sb, -1, style=wx.GA_HORIZONTAL | wx.GA_SMOOTH)
    ...
    def abort_run(self, verbose=True):
        if self.mp_method == "python":
            self.pids = easy_run.fully_buffered(
                "pgrep -u {} {}" "".format(user, python)
            ).stdout_lines
    ...
    def run(self):
        ...
        self.prime_process = thr.PRIMEThread(
            self,
            self.prime_file,
            self.out_file,
    ...
    def plot_final_results(self):
        ...
        self.summary_tab = SummaryTab(self.prime_nb, self.pparams, info)
    ...
    def final_step(self):
        ...
        if self.aborted:
            ...
            # Make the folder
            fin_folder = "merged_dataset_{}".format(
                os.path.basename(self.pparams.run_no)
            )
            ...
            shutil.copyfile(
                src=os.path.join(src_dir, src_mtz), dst=os.path.join(dst_dir, dst_mtz)
            )
            src_hkl = "postref_cycle_{}_merge.hkl".format(self.current_cycle)
            dst_hkl = "run_{:03d}_final_merged.hkl".format(run_no)
            shutil.copyfile(
                src=os.path.join(src_dir, src_hkl), dst=os.path.join(dst_dir, dst_hkl)
            )
            dst_log = "run_{}_log.txt".format(run_no)
            shutil.copyfile(
                src=os.path.join(src_dir, "log.txt"), dst=os.path.join(dst_dir, dst_log)
            )
...
class FileListCtrl(ct.CustomListCtrl):
    ...
    def __init__(self, parent, size=(-1, 300)):
        ...
        self.sizer.Add(
            self.button_sizer, pos=(1, 0), flag=wx.TOP | wx.BOTTOM, border=10
        )
    ...
    def set_type_choices(self, path):
        ...
        if os.path.isdir(path):
            ...
            if file_type in ("processed pickle", "reference MTZ", "sequence"):
                type_choices.extend(["processed pickle", "reference MTZ", "sequence"])

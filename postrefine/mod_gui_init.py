...
class PRIMEWindow(wx.Frame):
    def __init__(self, parent, id, title, phil=None, prefix="prime"):
        ...
        self.Bind(wx.EVT_BUTTON, self.onIsoRef, self.input_window.ref_box.btn_browse)
...
class PRIMEInputWindow(wx.Panel):
    ...
    def __init__(self, parent, phil=None):
        ...
        # Input file box
        self.inp_box = ct.InputCtrl(
            self, label="Input: ", label_size=(120, -1), label_style="bold", button=True
        )
        vbox.Add(self.inp_box, flag=wx.LEFT | wx.TOP | wx.RIGHT | wx.EXPAND, border=15)
        ...
        vbox.Add(self.out_box, flag=wx.LEFT | wx.TOP | wx.RIGHT | wx.EXPAND, border=15)
        ...
        self.ref_box = ct.InputCtrl(
            self, label="Reference: ", label_size=(120, -1), button=True
        )
        vbox.Add(self.ref_box, flag=wx.LEFT | wx.TOP | wx.RIGHT | wx.EXPAND, border=15)

        # Title box
        self.title_box = ct.InputCtrl(self, label="Job Title: ", label_size=(120, -1))
        vbox.Add(
            self.title_box, flag=wx.LEFT | wx.TOP | wx.RIGHT | wx.EXPAND, border=15
        )
        ...
        self.opt_spc_nres = wx.SpinCtrl(self, value="500", max=(15000), size=(80, -1))
        ...
        vbox.Add(opt_box, flag=wx.LEFT | wx.TOP | wx.RIGHT | wx.EXPAND, border=15)
    ...
    def onAdvancedOptions(self, e):
        advanced = PRIMEAdvancedOptions(
            self, title="Advanced PRIME Options", style=wx.DEFAULT_DIALOG_STYLE
        )
        advanced.Fit()
...
class PRIMEAdvancedOptions(wx.Dialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.uc_override = wx.CheckBox(self, label="Unit cell override")
        ...
        main_sizer.Add(dialog_box, flag=wx.EXPAND | wx.ALIGN_RIGHT | wx.ALL, border=15)
...
class PRIMEThread(Thread):
    ...
    def __init__(self, parent, prime_file, out_file):
...
class RuntimeTab(wx.Panel):
    ...
    def draw_plots(self, info, total_cycles):
        ...
        self.bcc_axes.clear()
        self.bcc_axes.bar(
            bins, info["binned_cc12"][-1], color="#2b8cbe", alpha=0.5, width=1, lw=0
        )
        self.bcc_axes.step(bins, info["binned_cc12"][-1], color="blue", where="post")
        ...
        self.bcomp_axes.step(
            bins, info["binned_completeness"][-1], color="red", where="post"
        )
        ...
        self.bmult_axes.step(
            bins, info["binned_n_obs"][-1], color="orange", where="post"
        )
...
class SummaryTab(wx.Panel):
    def __init__(self, parent, pparams, info):
        ...
        self.tb1 = ct.TableCtrl(
            self, rlabels=self.tb1_labels, contents=self.tb1_data, label_style="bold"
        )
        ...
        if save_dlg.ShowModal() == wx.ID_OK:
            with open(save_dlg.GetPath(), "a") as tb1_file:
                for i in range(len(self.tb1_data)):
                    line = u"{:<25} {:<40}\n".format(
                        self.tb1_labels[i].decode("utf-8"), self.tb1_data[i][0]
                    ).encode("utf-8")
                    tb1_file.write(line)
...
class PRIMERunWindow(wx.Frame):
    ...
    def __init__(self, parent, id, title, params, prime_file, out_file):
        wx.Frame.__init__(
            self,
            parent,
            id,
        ...
        self.tb_btn_abort = self.prime_toolbar.AddLabelTool(
            wx.ID_ANY,
            label="Abort",
    ...
    def onTimer(self, e):
        ...
        mtzs = [
            i
            for i in os.listdir(self.pparams.run_no)
            if ("cycle" in i and i.endswith("mtz"))
        ]

...
def process_input(argv=None):
    ...
    if params.target_crystal_system is not None:
        if params.target_crystal_system not in crystal_system_dict:
            raise Sorry(
                "Incorrect target_crystal_system (available options: Triclinic, Monoclinic, Orthorhombic, Tetragonal, Trigonal, Hexagonal, or Cubic)."
            )

    # check n_residues
    if params.n_residues is None:
        raise Sorry(
            "We have a new required parameter n_residues. Please specify number of residues of your structure in asymmetric unit (n_residues = xxx)."
        )

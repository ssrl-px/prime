...
def calc_partiality_anisotropy_set(
    ...
    ri_set = flex.double()
    for miller_index, bragg_angle, alpha_angle, spot_pred_x_mm, spot_pred_y_mm in zip(
        miller_indices,
        bragg_angle_set,
        alpha_angle_set,
        spot_pred_x_mm_set,
...
def func(params, *args):
    ...
    sin_theta_over_lambda_sq = (
        miller_array_o.two_theta(wavelength=wavelength)
        .sin_theta_over_lambda_sq()
        .data()
        .as_numpy_array()
    )
    ...
    partiality_model_flex, delta_xy_flex, ri_set = calc_partiality_anisotropy_set(
        uc,
        rotx,
        roty,
    ...
    if miller_array_o.d_min() < b_refine_d_min:
        I_o_full = (
            G * np.exp(-2 * B * sin_theta_over_lambda_sq) * I_o
        ) / partiality_model
    else:
...
class leastsqr_handler(object):
    ...
    def optimize(
        ...
        for i_sub_cycle in range(iparams.n_postref_sub_cycle):
            ...
            for j_refine_step in range(len(refine_steps)):
                ...
                if refine_mode == "scale_factor":
                    ...
                    pr_partiality_min = (
                        iparams.postref.crystal_orientation.partiality_min
                    )
                elif refine_mode == "reflecting_range":
                ...
                i_sel_sigmas = (
                    ...
                    / observations_original_sel.sigmas()
                ) > pr_sigma_min
                observations_original_sel = observations_original_sel.customized_copy(
                    indices=observations_original_sel.indices().select(i_sel_sigmas),
                    data=observations_original_sel.data().select(i_sel_sigmas),
                ...
                if j_refine_step == 0 and i_sub_cycle == 0:
                    if pres_in is None:
                        ...
                        a, b, c, alpha, beta, gamma = (
                            crystal_init_orientation.unit_cell().parameters()
                        )
                    else:
                        G = pres_in.G
                        B = pres_in.B
                ...
                if refine_mode == "scale_factor":
                    ...
                    if (abs(xopt[0] * 180 / math.pi) <= 2) and (
                        abs(xopt[1] * 180 / math.pi) <= 2
                    ):
                        rotx, roty = xopt
                    ...
                    if (
                        abs(xopt_uc[0] - iparams.target_unit_cell.parameters()[0])
                        <= (pr_uc_tol * iparams.target_unit_cell.parameters()[0] / 100)
                        and abs(xopt_uc[1] - iparams.target_unit_cell.parameters()[1])
                        <= (pr_uc_tol * iparams.target_unit_cell.parameters()[1] / 100)
                        and abs(xopt_uc[2] - iparams.target_unit_cell.parameters()[2])
                        <= (pr_uc_tol * iparams.target_unit_cell.parameters()[2] / 100)
                        and abs(xopt_uc[3] - iparams.target_unit_cell.parameters()[3])
                        <= (pr_uc_tol * iparams.target_unit_cell.parameters()[3] / 100)
                        and abs(xopt_uc[4] - iparams.target_unit_cell.parameters()[4])
                        <= (pr_uc_tol * iparams.target_unit_cell.parameters()[4] / 100)
                        and abs(xopt_uc[5] - iparams.target_unit_cell.parameters()[5])
                        <= (pr_uc_tol * iparams.target_unit_cell.parameters()[5] / 100)
                    ):
            ...
            if i_sub_cycle > 0:
                if (
                    (residuals[i_sub_cycle - 1][2] - residuals[i_sub_cycle][2]) < 0.1
                ) or (
        ...
        if pres_in is None:
            ...
            I_o_init = (
                1
                * np.exp(-2 * -0 * sin_theta_over_lambda_sq)
                * observations_original.data()
            ) / partiality_init
        else:
            partiality_init, delta_xy_init, ri_init = calc_partiality_anisotropy_set(
            ...
            I_o_init = (
                pres_in.G
                * np.exp(-2 * pres_in.B * sin_theta_over_lambda_sq)
                * observations_original.data()
            ) / partiality_init

        partiality_fin, delta_xy_fin, ri_fin = calc_partiality_anisotropy_set(
        ...
        if iparams.flag_plot_expert:
            ...
            plt.title("CC_init=%6.2f" % (CC_init))
            plt.xlabel("I_ref")
            plt.ylabel("I_obs")
            plt.subplot(132)
            plt.scatter(I_r_flex, I_o_fin, s=10, marker="o", c="b")
            plt.title("CC_final=%6.2f" % (CC_final))
            plt.xlabel("I_ref")
            plt.ylabel("I_obs")
            ...
            plt.plot(
                ...
                linewidth=2.0,
                c="b",
                label="After post-refinement <delta_xy>=%4.2f" % np.mean(delta_xy_fin),
            )

...
class merge_data_handler(object):
    ...
    def __init__(
        ...
        self.I_even, self.I_odd, self.I_even_h, self.I_odd_h, self.I_even_k, self.I_odd_k, self.I_even_l, self.I_odd_l = (
            I_two_halves_tuple
        )
    ...
    def generate_miller_array_from_miller_set(self, miller_set, target_anomalous_flag):
        self.miller_array_merge = (
            miller_set.array()
            .customized_copy(
                indices=self.miller_indices_merge,
                data=self.I_merge,
                sigmas=self.sigI_merge,
    ...
    def get_cc_anom(self):
        ...
        if self.miller_array_merge.anomalous_flag():
            ...
            cc_anom_acentric = flex.linear_correlation(
                ...
                ma_anom_dif_odd.data().select(i_acentric),
            ).coefficient()
    ...
    def get_cciso(self, miller_array_iso):
        ...
        if miller_array_iso:
            ...
            if len(matches_iso.pairs()) > 0:
                cciso = flex.linear_correlation(I_merge_match_iso, I_iso).coefficient()
    ...
    def reduce_to_cone_on_axis(self, axis_point_2, fraction_percent):
        miller_array_reduced = self.miller_array_merge.remove_cone(
            fraction_percent, axis_point_2=axis_point_2, negate=True
        )

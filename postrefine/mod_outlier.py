...
class outlier_handler(object):
    ...
    def good_i_flags(self, miller_array_i, iparams, flag_show_summary=False):
        ...
        try:
            ...
            i_seq_acentric_outliers = i_seq_acentric.select(
                data_e_acentric >= e_lim_acentric
            )
            i_seq_centric_outliers = i_seq_centric.select(
                data_e_centric >= e_lim_centric
            )

            i_seq_outliers = sorted(
                i_seq_acentric_outliers.concatenate(i_seq_centric_outliers)
            )

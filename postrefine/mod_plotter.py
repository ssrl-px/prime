...
class Plotter(IOTABasePanel):
    ...
    def __init__(
        self, parent, info, output_dir=None, anomalous_flag=False, *args, **kwargs
    ):
    ...
    def table_one(self):
        ...
        uc_edges = "{:4.2f}, {:4.2f}, {:4.2f}".format(
            self.info["mean_a"][-1], self.info["mean_b"][-1], self.info["mean_c"][-1]
        )
        uc_angles = "{:4.2f}, {:4.2f}, {:4.2f}".format(
            self.info["mean_alpha"][-1],
            self.info["mean_beta"][-1],
            self.info["mean_gamma"][-1],
        )
        res_total = "{:4.2f} - {:4.2f}".format(
            self.info["total_res_max"][-1], self.info["total_res_min"][-1]
        )
        res_last_shell = "{:4.2f} - {:4.2f}" "".format(
        ...
        t1_data = [
            ...
            ["{}".format(n_frames_bad)],
            ["{}".format(self.info["space_group_info"][-1].symbol_and_number())],
            [""],
            ["{}".format(uc_edges)],
            ["{}".format(uc_angles)],
            ...
            [
                "{:4.2f} ({:4.2f})".format(
                    self.info["total_n_obs"][-1], self.info["binned_n_obs"][-1][-1]
                )
            ],
            [
                "{:4.2f} ({:4.2f})".format(
            ...
            [
                "{:4.2f} ({:4.2f})".format(
                    self.info["total_cc12"][-1], self.info["binned_cc12"][-1][-1] * 100
                )
            ],
            [
                "{:4.2f} ({:4.2f})".format(
                    self.info["total_rmerge"][-1], self.info["binned_rmerge"][-1][-1]
                )
            ],
        ]
    ...
    def stat_charts(self):
        ...
        ax_cc12.legend(
            [start, end], labels, loc="lower left", fontsize=9, fancybox=True
        )
        ...
        ax_comp.grid(True)
        start, = ax_comp.plot(
            bins, self.info["binned_completeness"][0], c="#7fcdbb", lw=2
        )
        end, = ax_comp.plot(
            bins, self.info["binned_completeness"][-1], c="#2c7fb8", lw=3
        )
        labels = ["Initial", "Final"]
        ax_comp.legend(
            [start, end], labels, loc="lower left", fontsize=9, fancybox=True
        ...
        ax_i_sigi.grid(True)
        start, = ax_i_sigi.plot(
            bins, self.info["binned_i_o_sigi"][0], c="#7fcdbb", lw=2
        )
        end, = ax_i_sigi.plot(bins, self.info["binned_i_o_sigi"][-1], c="#2c7fb8", lw=3)
        labels = ["Initial", "Final"]
        ax_i_sigi.legend(
            [start, end], labels, loc="lower left", fontsize=9, fancybox=True
        )

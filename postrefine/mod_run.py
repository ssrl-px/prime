...
class run_handler(object):
    ...
    def count_frames(self):
        ...
        if self.iparams.queue.mode is None:
            return len(
                [fname for fname in os.listdir(self.iparams.run_no + "/pickles/")]
            )
        else:
            return len(
                [
                    fname
                    for fname in os.listdir(self.iparams.run_no + "/pickles/")
                    if os.path.getmtime(self.iparams.run_no + "/pickles/" + fname)
                    - self.pg_starts
                    > 0
                ]
            )
    ...
    def check_done(self, iparams, n_frames):
        ...
        while (time.time() - self.pg_starts < iparams.timeout_seconds) and (
            self.count_frames() < n_frames
        ):
            n_frames_tmp = self.count_frames()
            if n_frames_tmp > n_frames_now:
                n_frames_now = n_frames_tmp
                print "%10.0f frames processed %10.1f seconds" % (
                    n_frames_now,
                    time.time() - self.pg_starts,
                )

...
class intensities_scaler(object):
    ...
    def calc_avg_I(
        ...
        if multiplicity == 1:
            ...
            r_meas_top = flex.sum((I_full - I_avg) ** 2) * math.sqrt(
                n_obs / (n_obs - 1)
            )
    ...
    def prepare_output(self, results, iparams, avg_mode):
                    ...
                    if pres.R_xy_final <= (mean_R_xy_final * 6):
                        if pres.CC_final >= cc_thres:
                            ...
                            if (
                                abs(
                                    pres.uc_params[0]
                                    - iparams.target_unit_cell.parameters()[0]
                                )
                                <= (
                                    iparams.merge.uc_tolerance
                                    * iparams.target_unit_cell.parameters()[0]
                                    / 100
                                )
                                and abs(
                                    pres.uc_params[1]
                                    - iparams.target_unit_cell.parameters()[1]
                                )
                                <= (
                                    iparams.merge.uc_tolerance
                                    * iparams.target_unit_cell.parameters()[1]
                                ...
                                and abs(
                                    pres.uc_params[2]
                                    - iparams.target_unit_cell.parameters()[2]
                                )
                                <= (
                                    iparams.merge.uc_tolerance
                                    * iparams.target_unit_cell.parameters()[2]
                                ...
                                and abs(
                                    pres.uc_params[3]
                                    - iparams.target_unit_cell.parameters()[3]
                                )
                                <= (
                                    iparams.merge.uc_tolerance
                                    * iparams.target_unit_cell.parameters()[3]
                                ...
                                and abs(
                                    pres.uc_params[4]
                                    - iparams.target_unit_cell.parameters()[4]
                                )
                                <= (
                                    iparams.merge.uc_tolerance
                                    * iparams.target_unit_cell.parameters()[4]
                                ...
                                and abs(
                                    pres.uc_params[5]
                                    - iparams.target_unit_cell.parameters()[5]
                                )
                                <= (
                                    iparams.merge.uc_tolerance
                                    * iparams.target_unit_cell.parameters()[5]
                        ...
                        print pres.frame_no, img_filename, " discarded - target R (x,y) too high (R=%5.2f%%)" % (
                            pres.R_xy_final * 100
                        )
                        cn_bad_R_xy += 1
                else:
                    print pres.frame_no, img_filename, " discarded - target R too high (R=%5.2f%%)" % (
                        pres.R_final * 100
                    )
                    cn_bad_R += 1
        ...
        pr_params_mean, pr_params_std = self.calc_mean_postref_parameters(
            filtered_results
        )
        ...
        txt_out += " Rot.x:                    %12.2f (%7.2f)\n" % (
            pr_params_mean[6] * 180 / math.pi,
            pr_params_std[6] * 180 / math.pi,
        )
        txt_out += " Rot.y:                    %12.2f (%7.2f)\n" % (
            pr_params_mean[7] * 180 / math.pi,
            pr_params_std[7] * 180 / math.pi,
        )
        ...
        txt_out += (
            " unit cell:                %5.2f(%5.2f) %5.2f(%5.2f) %5.2f(%5.2f)\n"
    ...
    def write_output(
        ...
        for i in range(1, iparams.n_bins + 1):
            ...
            if len(miller_array_merge.data().select(i_binner)) > 0:
                ...
                i_filter = (
                    flex.abs((I_obs_bin - np.median(I_obs_bin)) / np.median(I_obs_bin))
                    < sigma_filter
                )

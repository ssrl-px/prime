...
class intensities_scaler(object):
    ...
    def write_output(self, mdh, iparams, output_mtz_file_prefix, avg_mode):
        ...
        if iparams.flag_hush:
            ...
            sp_res = (
                sp_complete
            ) = sp_n_obs = sp_cc12 = sp_rmerge = sp_i_o_sigi = sp_isqr = []
            # binning
    ...
    def plot_stats(self, results, iparams):
        ...
        if iparams.flag_plot:
            ...
            for i in xrange(len(params)):
                ...
                plt.hist(
                    params_array[:, i], num_bins, normed=0, facecolor="green", alpha=0.5
                )
                ...
                plt.title(
                    ...
                    % (
                        np.mean(params_array[:, i]),
                        np.median(params_array[:, i]),
                        np.std(params_array[:, i]),
                    )
                )
            plt.show()

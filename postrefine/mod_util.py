...
class intensities_scaler(object):
    ...
    def write_output(
        ...
        miller_array_merge = miller_set.array(data=I_merge, sigmas=sigI_merge)
        if iparams.flag_normalized:
            miller_array_merge = (
                miller_array_merge.set_observation_type_xray_amplitude()
            )
            data_label = "FOBS"
        else:
            miller_array_merge = (
                miller_array_merge.set_observation_type_xray_intensity()
            )
            data_label = "IOBS"

        miller_array_template_asu = miller_array_merge.complete_array().resolution_filter(
        ...
        wp = statistics.wilson_plot(
            miller_array_merge_amplitude, asu_contents, e_statistics=True
        )
        ...
        if iparams.flag_normalized:
            ...
            sin_theta_over_lambda_sq = (
                miller_array_merge.two_theta(wavelength=wavelength_mean)
                .sin_theta_over_lambda_sq()
                .data()
            )
            f_from_e = flex.double(
                miller_array_merge.data()
                / (G * np.exp(-2 * B * sin_theta_over_lambda_sq))
            )
            sigf_from_sige = flex.double(
                miller_array_merge.sigmas()
                / (G * np.exp(-2 * B * sin_theta_over_lambda_sq))
            )
            miller_array_merge = miller_array_merge.customized_copy(
                data=f_from_e, sigmas=sigf_from_sige
            )

        # Remove outliers
        if iparams.flag_outlier_rejection:
            normalised = miller_array_merge_amplitude.normalised_amplitudes(
            ...
            print "Outlier detection: ", len(miller_array_merge.indices()) - len(
            ...
            miller_array_merge_intermediate = miller_array_merge_intermediate.select(
                select_flags
            )
        ...
        for i in range(1, iparams.n_bins + 1):
            ...
            mean_i_over_sigi_bin, multiplicity_bin, r_meas_w_bin, r_meas_bin, n_refl_cc12_bin, r_split_bin, mean_I_sq_bin = (
                0,
                0,
                0,
                0,
            ...
            txt_out += (
                "%02d %7.2f - %7.2f %5.1f %6.0f / %6.0f %7.2f %7.2f %7.2f %7.2f %6.0f %7.2f %6.0f %7.2f %6.0f %9.2f %10.2f %6.2f %6.2f"
                % (
                    i,
        ...
        txt_out += "---------------------------------------------------------------------------------------------------------------------------------------------------\n"
        txt_out += (
            "        TOTAL        %5.1f %6.0f / %6.0f %7.2f %7.2f %7.2f %7.2f %6.0f %7.2f %6.0f %7.2f %6.0f %9.2f %10.2f %6.2f %6.2f\n"
            % (

...
class postref_handler(object):
    ...
    def postrefine_by_frame(
        ...
        ), _tmp_pickle_filename[
            len(_tmp_pickle_filename) - 1
        ]
    ...
    def scale_frame_by_mean_I(self, frame_no, pickle_filename, iparams, mean_of_mean_I):
        ...
        _tmp_pickle_filename = pickle_filename.split("/")
        print "%6.0f %6.2f %6.0f %6.0f %14.2f %14.2f %14.2f %6.2f %6.2f" % (
            frame_no,
            observations_original.d_min(),
            len(observations_original.data()),
            len(observations_original_sel.data()),
            np.sum(observations_original_sel.data()),
            np.mean(observations_original_sel.data()),
            np.median(observations_original_sel.data()),
            G,
            B,

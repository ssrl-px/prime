...
class postref_handler(object):
    ...
    def organize_input(
        self, observations_pickle, iparams, pickle_filename=None, mode=None
    ):
        ...
        assert len(alpha_angle_obs) == len(observations.indices()), (
            "Size of alpha angles and observations are not equal %6.0f, %6.0f"
            % (len(alpha_angle_obs), len(observations.indices()))
        )
        ...
        if iparams.flag_LP_correction:
            ...
            for pred in mm_predictions:
                ...
                cos_two_polar_angle.append(
                    math.cos(2.0 * math.atan2(pred[1] - ybeam, pred[0] - xbeam))
                )
            ...
            P_nought_vec = 0.5 * (1.0 + cos_sq_tt_vec)
            ...
            observations = observations.customized_copy(
                data=observations.data() / (P_nought_vec - P_prime)
            )
        ...
        if iparams.flag_plot_expert:
            ...
            plt.title(
                "Intensity plot by resolutions (B-factor=%6.2f)" % (wilson_k_b[1])
            )
            plt.xlabel("1/d^2")
            plt.ylabel("<I>")
            plt.show()
        ...
        if iparams.flag_apply_b_by_frame:
            ...
            I_b = (
                flex.exp(-2 * wilson_b * sin_theta_over_lambda_sq) * observations.data()
            )
            sigI_b = (
                flex.exp(-2 * wilson_b * sin_theta_over_lambda_sq)
                * observations.sigmas()
            )
    ...
    def scale_frame_by_mean_I(self, frame_no, pickle_filename, iparams, mean_of_mean_I):
        ...
        if (
            iparams.target_pointgroup is not None
            and crystal_pointgroup != iparams.target_pointgroup
        ):
        ...
        if iparams.hklrefin is not None:
            ...
            if miller_array_ref is not None:
                ...
                miller_indices_original_match = flex.miller_index(
                    (
                        observations_original_sel.indices()[pair[1]]
                        for pair in matches.pairs()
                    )
                )
                I_obs_match = flex.double(
                    [
                        observations_original_sel.data()[pair[1]]
                        for pair in matches.pairs()
                    ]
                )
                ...
                sigI_obs_match = flex.double(
                    [
                        observations_original_sel.sigmas()[pair[1]]
                        for pair in matches.pairs()
                    ]
                )

                alpha_angle_set = flex.double(
                    [alpha_angle_sel[pair[1]] for pair in matches.pairs()]
                )
                spot_pred_x_mm_set = flex.double(
                    [spot_pred_x_mm_sel[pair[1]] for pair in matches.pairs()]
                )
                spot_pred_y_mm_set = flex.double(
                    [spot_pred_y_mm_sel[pair[1]] for pair in matches.pairs()]
                )
        ...
        print "frame %6.0f" % frame_no, "<I>=%9.2f <G>=%9.2f G=%9.2f B=%9.2f nrefl=%5.0f nrefl_used=%5.0f" % (
            ...
            G,
            B,
            len(observations_non_polar.data()),
            len(observations_non_polar_sel.data()),
        ), polar_hkl
        return pres
...
def write_output(
    ...
    # lenght of this list equals to number of input frames
    inten_scaler = intensities_scaler()
    miller_array_merge, txt_out, csv_out = inten_scaler.write_output(
        miller_indices_merge,
        I_merge,
        sigI_merge,
        stat_all,

...
class KnownValues(unittest.TestCase):
    ...
    y_model = (m * x) + c
    pickle_filename = (
        "test_data/int_shot-s00-E1_0_00002_20140518051704255_masked.pickle"
    )
    test_pickle = pickle.load(open(pickle_filename, "rb"))

    def testCoefficientOfDetermination(self):
        r_sqr = mod_leastsqr.coefficient_of_determination(
            flex.double(self.y), flex.double(self.y_model)
        )
        self.assertAlmostEqual(self.r_sqr, r_sqr)

    def testStandardErrorOfTheEstimate(self):
        se = mod_leastsqr.standard_error_of_the_estimate(
            flex.double(self.y), flex.double(self.y_model), self.n_params
        )
